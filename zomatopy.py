...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", limit=20,sort='rating',order='desc',start=0):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&start=" + str(start) + "&cuisines=" + str(cuisines)+"&sort="+sort +"&order="+order, headers=headers).content).decode("utf-8")
